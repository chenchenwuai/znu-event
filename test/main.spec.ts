...
test('has 是否正确?', () => {
  ...
  expect(eventEmitter.has('edf')).toBeFalsy()
})
...
test('eventNames是否正确返回所有事件名?', ()=>{
  ...
  expect(eventEmitter.eventNames()).toContain('tap');
  expect(eventEmitter.eventNames()).toContain('pan');
